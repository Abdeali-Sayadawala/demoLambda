...
for dir in lambda-*.prm; do # iterating all the prm files for each lambda functions
    ...
    if [ "$curr_lambda" != "" ]; then
        ...
        live_layer_arn=$(echo $curr_lambda | python -c 'import json,sys;print(",".join([layer["Arn"] for layer in json.load(sys.stdin)["Configuration"]["Layers"]])) if "Layers" json.load["Configuration"] else print("")')
